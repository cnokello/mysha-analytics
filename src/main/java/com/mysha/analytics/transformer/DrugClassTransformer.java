...
import com.mysha.analytics.business.DrugClassProcessor;
...
public class DrugClassTransformer {
  ...
  private static final Logger LOGGER = Logger.getLogger(DrugClassTransformer.class);
  ...
  public void initKafkaConsumer() {
    if (consumer == null) {
      try {
        ...
        topic = cfg.getEnv().getProperty("kafka.topic.drugs.class");
  ...
  public void transform() {
    ...
    try {
      ...
      for (final KafkaStream stream : streams) {
        ...
        executor.submit(new DrugClassProcessor(stream, threadNum, cassandra));
        threadNum++;
      }
