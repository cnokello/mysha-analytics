...
import com.mysha.analytics.business.DrugSideEffectProcessor;
...
public class DrugSideEffectTransformer {
  ...
  private static final Logger LOGGER = Logger.getLogger(DrugSideEffectTransformer.class);
  ...
  public void initKafkaConsumer() {
    if (consumer == null) {
      try {
        ...
        topic = cfg.getEnv().getProperty("kafka.topics.drugs.effect");
  ...
  public void transform() {
    ...
    try {
      ...
      for (final KafkaStream stream : streams) {
        ...
        executor.submit(new DrugSideEffectProcessor(stream, threadNum, cassandra));
        threadNum++;
      }
    } catch (Exception e) {
      LOGGER.error(String.format("Message: %s\nTrace: %s\n", e.getMessage(),
